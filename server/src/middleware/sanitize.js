...
// کلیدهای خطرناک prototype pollution — باید همیشه حذف شوند
const PROTO_KEYS = new Set(['__proto__', 'constructor', 'prototype']);
const isForbiddenKey = (key) => PROTO_KEYS.has(key) || FORBIDDEN_KEY.test(key);
...
function sanitizeInPlace(obj, depth = 0, dropOperatorValues = false) {
  ...
  // getOwnPropertyNames (نه Object.keys) تا کلید own به نام __proto__
  // که از JSON.parse('{"__proto__":...}') می‌آید هم دیده و حذف شود.
  for (const key of Object.getOwnPropertyNames(obj)) {
    if (isForbiddenKey(key)) {
      delete obj[key]; // حذف کلید آلوده (prototype pollution یا اپراتور مونگو)
      continue;
    }
    const val = obj[key];
